...
namespace Chi {
namespace SFM {
namespace InitImageListing {


/// Check that Kmatrix is a string like "f;0;ppx;0;f;ppy;0;0;1"
/// With f,ppx,ppy as valid numerical value
bool checkIntrinsicStringValidity(const std::string& Kmatrix, double& focal, double& ppx,
                                  double& ppy)
{
    ...
    if (exifReader) {
        ...
        if (exifReader->open(filename) && exifReader->doesHaveExifInfo()) {
            ...
            if (exifReader->GPSLatitude(&latitude) && exifReader->GPSLongitude(&longitude) &&
                ...
                case 1: pose_center = lla_to_utm(latitude, longitude, altitude); break;
                case 0:
                default: pose_center = lla_to_ecef(latitude, longitude, altitude); break;
                }
                return true;
            }
        }
...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        OPENMVG_LOG_INFO << "Usage: " << argv[0] << '\n'
                         ...
                         << "\t" << static_cast<int>(PINHOLE_CAMERA_RADIAL1)
                         << ": Pinhole radial 1\n"
                         << "\t" << static_cast<int>(PINHOLE_CAMERA_RADIAL3)
                         << ": Pinhole radial 3 (default)\n"
                         << "\t" << static_cast<int>(PINHOLE_CAMERA_BROWN) << ": Pinhole brown 2\n"
                         << "\t" << static_cast<int>(PINHOLE_CAMERA_FISHEYE)
                         << ": Pinhole with a simple Fish-eye distortion\n"
                         ...
                         << "[-W|--prior_weights] \"x;y;z;\" of weights for each dimension of the "
                            "prior (default: 1.0)\n"
    ...
    for (std::vector<std::string>::const_iterator iter_image = vec_image.begin();
        ...
        {
            if (!checkIntrinsicStringValidity(sKmatrix, focal, ppx, ppy)) focal = -1.0;
        }
        ...
        if (focal == -1) {
            ...
            if (bHaveValidExifMetadata)   // If image contains meta data
            {
                ...
                {
                    const std::string sCamModel =
                        exifReader->getBrand() + " " + exifReader->getModel();

                    Datasheet datasheet;
                    if (getInfo(sCamModel, vec_database, datasheet)) {
                        // The camera model was found in the database so we can compute it's
                        // approximated focal length
                        ...
                        error_report_stream << stlplus::basename_part(sImageFilename)
                                            ...
                                            << "\n"
                                            << "Please consider add your camera model and sensor "
        ...
        if (focal > 0 && ppx > 0 && ppy > 0 && width > 0 && height > 0) {
            ...
            case PINHOLE_CAMERA_RADIAL3:
                intrinsic = std::make_shared<Pinhole_Intrinsic_Radial_K3>(
                    width, height, focal, ppx, ppy, 0.0, 0.0, 0.0);   // setup no distortion as
                                                                      // initial guess
                break;
            ...
            case PINHOLE_CAMERA_FISHEYE:
                intrinsic = std::make_shared<Pinhole_Intrinsic_Fisheye>(
                    width, height, focal, ppx, ppy, 0.0, 0.0, 0.0, 0.0);   // setup no distortion as
                                                                           // initial guess
                break;
